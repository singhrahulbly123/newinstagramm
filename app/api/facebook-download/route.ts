...
export async function GET(req: Request) {
  try {
    ...
    if (isDev) console.log('facebook-download route hit');

    const parsedReqUrl = new URL(req.url);
    const { searchParams } = parsedReqUrl;
    if (isDev) console.log('facebook-download incoming querystring:', parsedReqUrl.search);
    const url = searchParams.get('url');
    const filenameParam = searchParams.get('filename');
    if (isDev) console.log('facebook-download incoming url param (raw):', url);

    if (!url) {
      if (isDev) console.log('facebook-download missing url param');
    ...
    try {
      ...
      if (isDev) console.log('facebook-download invalid url');
    ...
    const allowedHosts = ['facebook.com', 'www.facebook.com', 'm.facebook.com', 'fb.watch', 'fbsbx.com', 'cdn.fbsbx.com', 'fbcdn.net'];
    const okHost = host.endsWith('fbcdn.net') || allowedHosts.some((h) => host === h || host.endsWith(h));
    if (isDev) console.log('facebook-download host:', host, 'host validation result:', okHost);
    if (!okHost) {
      if (isDev) console.log('facebook-download invalid host:', host);
    ...
    if (isDev) console.log('facebook-download fetching upstream');
    ...
    if (isDev) console.log('facebook-download upstream status:', upstream.status);

    if (!upstream.ok) {
      if (isDev) console.log('facebook-download upstream not ok');
    ...
    if (isDev) console.log('facebook-download upstream content-type:', contentType);
