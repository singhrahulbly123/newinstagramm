...
export async function GET(request: Request) {
  ...
  if (!isAllowedHost(mediaUrl)) {
    return NextResponse.json({ error: 'Only supported media URLs are allowed.' }, { status: 400 });
  }
  ...
  const referer = mediaUrl.hostname.includes('facebook') || mediaUrl.hostname.includes('fbcdn') || mediaUrl.hostname.includes('fbsbx')
