...
let ffprobeStatic: { path: string } | null = null;
let ffprobeStaticPath: string | null = null;

try {
  ffprobeStatic = require('ffprobe-static') as { path: string };
  ffprobeStaticPath = ffprobeStatic.path ? path.resolve(ffprobeStatic.path) : null;
  console.log('[AUDIO] ffprobe-static package loaded', {
    ffprobeStaticPath,
    exists: ffprobeStaticPath ? existsSync(ffprobeStaticPath) : false,
  });
} catch (error) {
  console.log('[AUDIO] ffprobe-static package unavailable', { error: (error as Error).message });
}

const ffmpegStaticPath = ffmpegPath ? path.resolve(String(ffmpegPath)) : null;

// ============================================================
// STARTUP DIAGNOSTICS: Log ffprobe path availability
// ============================================================
if (ffprobeStaticPath) {
  const ffprobeExists = existsSync(ffprobeStaticPath);
  console.log('[AUDIO] Startup ffprobe path diagnostics', {
    ffprobeStaticPath,
    exists: ffprobeExists,
  });
} else {
  console.log('[AUDIO] Startup ffprobe path diagnostics: ffprobeStaticPath is null');
}
...
function getFFmpegStaticCandidates() {
  ...
  try {
    ...
    candidates.push(path.join(path.dirname(packageEntry), process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg'));
  } catch (error) {
    console.log('[AUDIO] ffmpeg-static resolve failed', { error: (error as Error).message });
  }
...
function getFFprobeStaticCandidates() {
  ...
  try {
    const packageEntry = require.resolve('ffprobe-static');
    candidates.push(
      path.resolve(
    ...
    console.log('[AUDIO] ffprobe-static resolve failed', { error: (error as Error).message });
  }
