...
export async function GET(req: Request) {
  try {
    ...
    if (isDev) console.log('[youtube-download] request:', { videoId, itag: itagParam, filename, mode });
    ...
    try {
      ...
      if (isDev) console.log('[youtube-download] getInfo error:', message);
    ...
    if (isDev) console.log('[youtube-download] selected itag:', itag, 'format found:', Boolean(selectedFormat));
    ...
    if (isDev) console.log('[youtube-download] upstream host:', upstreamHost);
    ...
    if (isDev) console.log('[youtube-download] upstream status:', upstreamResp.status);

    if (!upstreamResp.ok && upstreamResp.status !== 206) {
      const text = await upstreamResp.text().catch(() => '');
      if (isDev) console.log('[youtube-download] upstream error body snippet:', text.slice(0, 200));
    ...
    if (isDev) console.log('[youtube-download] using filename:', filename, 'mode:', mode);

    return new Response(upstreamResp.body, { status: upstreamResp.status, headers: responseHeaders });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    const isDev = process.env.NODE_ENV === 'development';
    if (isDev) console.log('[youtube-download] unexpected error:', message);
