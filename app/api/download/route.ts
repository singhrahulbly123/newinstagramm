...
const instaRegex = /^https?:\/\/(www\.)?instagram\.com\/(?:reel|reels|p|tv)\/[A-Za-z0-9_-]+(?:[\/?].*)?$/i;
...
export async function POST(request: Request) {
  ...
  const response = await fetch(pageUrl, { headers: instagramHeaders }).catch(() => null);
  ...
  // Instagram often changes the main page JSON while the embed endpoint still exposes media metadata.
  if (!downloadUrl) {
    try {
      const embedUrl = normalizeInstagramUrl(originalUrl).replace(/\/+$/, '') + '/embed/';
      const embedResp = await fetch(embedUrl, { headers: instagramHeaders }).catch(() => null);
      if (embedResp && embedResp.ok) {
        const embedHtml = await embedResp.text();
        downloadUrl = extractInstagramVideoUrl(embedHtml, originalUrl);
