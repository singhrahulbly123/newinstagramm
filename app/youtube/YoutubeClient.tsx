    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 text-slate-900 dark:from-slate-950 dark:to-slate-900 dark:text-white">
      <div className="mx-auto max-w-6xl px-4 py-8">
        <section className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-800">▶️ YouTube Downloader</div>
        ...
        <section className="mt-6 rounded-2xl border bg-white p-6">
          <form onSubmit={handleExtract} className="flex flex-col gap-3 sm:flex-row">
            ...
            <button
              type="submit"
              disabled={loading}
              className="rounded-lg bg-gradient-to-r from-stone-950 via-emerald-950 to-teal-900 px-6 py-3 text-white disabled:opacity-50"
            >
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
                  ...
                  <div className="mt-4 rounded-lg bg-slate-950 text-white">
                    {previewUrl && selectedFormat?.hasVideo && selectedFormat?.hasAudio ? (
                      ...
                      <Image
                        ...
                        alt={result.title || 'YouTube preview'}
                        width={400}
                        height={225}
              ...
              <aside className="rounded-lg border p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {result.formats?.map((format) => (
                    <button
                      ...
                      className={`rounded-md px-3 py-2 text-left text-sm font-semibold ${selectedItag === format.itag ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-900'}`}
                    >
                ...
                <div className="mt-4">
                  <a
                    href={downloadUrl ?? '#'}
                    className="block w-full text-center rounded-md bg-gradient-to-r from-stone-950 via-emerald-950 to-teal-900 px-4 py-3 text-white"
                  >
