...
export default function ProfilePictureDownloader() {
  ...
  const helperText = useMemo(() => {
    ...
    if (items.length > 1) return 'Multiple images found. You can download all images or save individual files below.';
    if (items.length === 1) return 'Profile picture ready. Tap download to save the original image.';
    return 'Paste an Instagram profile URL to preview the highest resolution profile picture.';
  }, [items.length, status]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!url.trim()) {
      setStatus('Please paste an Instagram profile URL first.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data.success || !data.items || data.items.length === 0) {
        setStatus(data.error || 'Unable to extract the profile picture. Please check the URL and try again.');
        return;
      }
  ...
  return (
    <div className="space-y-5">
      <form onSubmit={handleSubmit} className="grid gap-3 rounded-[2rem] border border-slate-200 bg-white px-5 py-5 shadow-sm shadow-slate-200/50 sm:grid-cols-[1fr_auto] sm:items-center">
        <label htmlFor="instagram-profile-url" className="sr-only">
        ...
        <button
          type="submit"
          disabled={loading}
          className="inline-flex h-14 items-center justify-center rounded-[1.75rem] bg-gradient-to-r from-stone-950 via-emerald-950 to-teal-900 px-8 text-sm font-semibold text-white shadow-lg shadow-emerald-300/30 transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {loading ? 'Loading…' : 'Preview Profile Picture'}
        </button>
      </form>
      ...
      {previewUrl ? (
        <div className="space-y-5 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/50 dark:border-slate-700 dark:bg-slate-950">
          ...
          <div className="relative overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-900" style={{ minHeight: 240 }}>
            <Image
              src={previewUrl}
              alt="Instagram profile picture preview"
              fill
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-600">
              Showing {items.length} image{items.length > 1 ? 's' : ''} from this profile.
            </p>
            {items.length > 1 ? (
              <button
                type="button"
     ...
     <DownloadSection />
      <div className="rounded-[2rem] border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600 shadow-sm shadow-slate-200/50">
        ...
        <ul className="mt-3 space-y-2 list-disc pl-5">
          <li>Only download profile pictures from public Instagram profiles.</li>
          <li>Respect creator rights and use downloaded images for personal use.</li>
          <li>If a link is not working, confirm it is a valid Instagram profile URL.</li>
        </ul>
      </div>
    </div>
  );
