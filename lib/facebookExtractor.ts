...
const FACEBOOK_SHARE_REGEX = /^https?:\/\/(?:www\.)?facebook\.com\/share\/.+$/i;
const FACEBOOK_STORY_REGEX = /^https?:\/\/(?:www\.|m\.)?facebook\.com\/stories\/.+$/i;
const FACEBOOK_STORY_PHP_REGEX = /^https?:\/\/(?:www\.|m\.)?facebook\.com\/story\.php\?.*story_fbid=/i;
const FACEBOOK_GENERIC_REGEX = /^https?:\/\/(?:www\.|m\.)?facebook\.com\/.+/i;
...
function buildFacebookPageCandidates(url: string): string[] {
  ...
  try {
    ...
    if (parsed.hostname.includes('facebook.com') && segments[0] === 'stories' && segments.length >= 2) {
      const ownerId = segments[1];
      const storyId = segments[2] || '';
      if (ownerId && storyId) {
        candidates.add(`https://www.facebook.com/story.php?story_fbid=${encodeURIComponent(storyId)}&id=${encodeURIComponent(ownerId)}`);
        candidates.add(`https://m.facebook.com/story.php?story_fbid=${encodeURIComponent(storyId)}&id=${encodeURIComponent(ownerId)}`);
        candidates.add(`https://www.facebook.com/stories/${ownerId}/${storyId}/?view_single=1`);
        candidates.add(`https://m.facebook.com/stories/${ownerId}/${storyId}/?view_single=1`);
      }
    }

    if (parsed.hostname.includes('facebook.com') && parsed.pathname.includes('/share/')) {
      candidates.add(trimmed.replace(/^https?:\/\/(www\.)?facebook\.com/i, 'https://m.facebook.com'));
    }
  } catch {
    // ignore invalid URLs here and fall back to the original string
  }
...
function collectQualityVariants(html: string, debug: string[]): Map<string, string> {
  ...
  // Fallback: raw media URL search (MP4 and Facebook video/CDN hosts)
  const mediaMatches = Array.from(html.matchAll(/https?:\/\/[^"'<><\s\\]+/gi));
  debug.push(`Raw media URL matches: ${mediaMatches.length}`);
  if (qualities.size === 0 && mediaMatches.length > 0) {
    const urls = mediaMatches
      .map((match) => decodeUrlFully(match[0], debug))
      .filter((url) =>
        url.startsWith('https://') &&
        (url.includes('.mp4') || url.includes('fbsbx') || url.includes('fbcdn') || url.includes('/video/'))
      );
    if (urls.length > 0) {
      qualities.set('SD', urls[0]);
      debug.push(`✓ SD from raw media search: ${urls[0].substring(0, 80)}...`);
    }
  }
...
export function isSupportedFacebookUrl(url: string): boolean {
  const trimmed = url.trim();
  return (
    FACEBOOK_REEL_REGEX.test(trimmed) ||
    FACEBOOK_VIDEO_REGEX.test(trimmed) ||
    FACEBOOK_SHARE_REGEX.test(trimmed) ||
    FACEBOOK_STORY_REGEX.test(trimmed) ||
    FACEBOOK_GENERIC_REGEX.test(trimmed) ||
    FB_WATCH_REGEX.test(trimmed) ||
    MOBILE_FACEBOOK_REGEX.test(trimmed)
  );
}

export function normalizeFacebookUrl(url: string): string {
  const candidates = buildFacebookPageCandidates(url);
...
export async function fetchFacebookPageHtml(pageUrl: string, retryCount: number = 0): Promise<string> {
  ...
  const candidateUrls = buildFacebookPageCandidates(pageUrl);
  ...
  try {
    for (const candidateUrl of candidateUrls) {
      try {
        ...
        const isBlockedPage =
          lowerHtml.includes('sorry, something went wrong') ||
          lowerHtml.includes('log in to facebook') ||
          lowerHtml.includes('log into facebook') ||
          lowerHtml.includes('this content isn\'t available right now') ||
          lowerHtml.includes('this page isn\'t available');

        if (response.ok && !isBlockedPage) {
          return html;
        }

        if (response.status >= 400) {
          log('FACEBOOK', `Fetch failed for ${candidateUrl} with status ${response.status}`, { retryCount });
        }
      } catch (err) {
        lastError = err;
      }
    ...
    if (retryCount < MAX_RETRIES) {
      log('FACEBOOK', `All Facebook fetch candidates failed, retrying...`, { retryCount });
      await new Promise((resolve) => setTimeout(resolve, 1000 * (retryCount + 1)));
      return fetchFacebookPageHtml(pageUrl, retryCount + 1);
    }

    throw lastError instanceof Error ? lastError : new Error(`Unable to fetch Facebook content for ${pageUrl}`);
  } catch (err) {
...
async function extractFacebookWithPlaywright(
  ...
  try {
    ...
    try {
      ...
      debug.push(`Navigation response status: ${response?.status() ?? 'unknown'}`);
      debug.push(`Resolved page URL: ${resolvedPageUrl}`);
    } catch (navErr) {
