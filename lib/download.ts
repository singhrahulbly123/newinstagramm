...
export function extractVideoUrlFromJsonHtml(html: string) {
  ...
  const snippet = html.slice(start, start + 12000);
...
export function extractInstagramVideoUrlResult(html: string, originalUrl: string): InstagramVideoExtractionResult {
  ...
  const directVideoVersionUrl = extractVideoUrlFromJsonHtml(html);
  if (directVideoVersionUrl) {
    diagnostics.push('[AUDIO] Strategy video_versions raw HTML success');
    console.log('[AUDIO] Strategy video_versions raw HTML success');
    return { videoUrl: directVideoVersionUrl, source: 'video_versions', diagnostics };
  }
